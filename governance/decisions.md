...
- Status: RECORDED (round-1 lineup + naming-decoupling binding; `elecrow5` board bring-up + EPIC-05 retarget + hardware-doc updates + CI FILED — S-05-041…047; round-2 in-house branding deferred). Radio posture in point 4 REFINED 2026-07-14 to owner V1 spec: HaLow mandatory on every V1 device; LoRa/GPS/compass/C6 are additive-optional sub-variants (never swaps for HaLow); firmware supports all combinations, products always ship HaLow.
...
## D-0027 — `elecrow5` schematic-validated hardware truth (corrects D-0026 display interface)

- Date: 2026-07-18 (owner-directed, this session)
- Decision: Netlist-level validation of the Elecrow CrowPanel Advance 5″ ESP32-P4's **own** Eagle schematic (`ESP32-P4 Display 5.0 inch V1.0.sch`, in the board repo now mirrored under `vendor/elecrow/`) establishes the `elecrow5` hardware truth and **corrects two assumptions carried in D-0026 clause (2)** that came from generic ESP32-P4 web specs rather than this board. CONFIRMED (with net evidence): (1) **Display is 16-bit parallel RGB565, NOT MIPI-DSI** — 40-pin FPC JP1, R/G/B bit lines on GPIO4–19, PCLK=GPIO3, DE=GPIO2, HSYNC=GPIO40, VSYNC=GPIO41 (this supersedes "MIPI-DSI" in D-0026 §(2) and drives an `esp_lcd` **RGB** panel path, not DSI). (2) **Capacitive touch is on-panel over I²C1** (SCL=GPIO46, SDA=GPIO45, INT=GPIO42, RST via GPIO36 + the STC8 co-MCU) — the controller IC is on the display flex and **not in the board BOM, so "GT911" is UNVERIFIED** (likely GT911 @ 0x5D/0x14; confirm at bring-up). (3) **C6↔P4 bus = SDIO 4-bit** (ESP32-C6-MINI-1 as an SDIO radio slave: CLK/CMD/D0–D3 = P4 GPIO53/54/52/51/50/49; C6 UART0 only on flashing test-pads) — this **confirms** the D-0026 §(7) "assume SDIO/ESP-Hosted" open item; ESP-IDF ≥ v5.3 + `esp_hosted`/`esp_wifi_remote`. (4) **Module (HaLow/LoRa) slot = SPI2** (SCK=GPIO26, MOSI=GPIO48, MISO=GPIO47) on headers J9/J11, plus GPIO29/30 (reset/BUSY, SX126x-style — the LoRa sub-variant path, S-05-047), UART2, I²C1, 3V3/5V; **MOSI/MISO are shared with UART1→Crowtail J2 via an SGM3005 analog mux selected by slide-switch S1 + the STC8** — firmware must set the mux before using the slot. (5) **No on-board GNSS, magnetometer/IMU, or fuel gauge** — all external (Crowtail UART/I²C); battery (JST-PH2.0, J3) is sensed by the **STC8H1K08 co-MCU** ADC, read by the P4 over I²C1. (6) **STC8H1K08 (U14) is a power/peripheral co-MCU** the P4 must talk to over I²C1: it gates backlight-EN, the SPI/UART mux select, touch-RST, camera-RST, audio-shutdown, charge LEDs, and VBAT/charge-status sense — a first-class bring-up dependency (new story S-05-048). (7) **Camera = external only:** 2-lane MIPI-CSI header (FPC3), SCCB on I²C2 (GPIO34/33), no on-board sensor — the OV5647/SC2336 choice is a module question, not a PCB part (scopes S-05-045). (8) Also confirmed: **2× NS4168** I²S amps (shared BCLK GPIO22) + **PDM MEMS mic**; **W25Q128JV 16 MB** flash; dual USB-C (**CH340K** UART-boot + P4-native USB 2.0). All EPIC-05 `elecrow5` stories (S-05-041..047) are updated to these facts and `docs/dev/OMEGA_HW_BASELINE.md` records the validated `elecrow5` truth.
- Rationale: Owner direction 2026-07-18 to validate/execute with nothing assumed. Reading the board's own netlist (not the Espressif EV-board reference, whose SDIO pattern holds but whose MIPI-DSI panel does not) caught the RGB-vs-DSI error before it propagated into driver work, confirmed the SDIO bus, and surfaced the STC8 co-MCU as a required bring-up peer. Recorded as a corrective entry (not a history edit) exactly as D-0021 corrected D-0020's baseline reads. Facts only — pin/net data, no reproduction of the copyrighted schematic.
- Status: RECORDED (corrects D-0026 §(2) display=RGB565-parallel + touch-controller-unverified; confirms SDIO; adds S-05-048 for the STC8 co-MCU; EPIC-05 stories + `OMEGA_HW_BASELINE.md` updated). Bring-up UNKNOWNS remaining: exact touch controller/addr, TLV62569 rail voltages, module-slot LoRa module pinout, PSRAM size (P4NRW32 implies integrated).
- Reference: D-0020, D-0021, D-0026, `docs/dev/OMEGA_HW_BASELINE.md`, `vendor/elecrow/` (README + `_MANIFEST.md` provenance; `esp32-p4-function-ev-board-schematics` cross-ref), EPIC-05 (S-05-041..048), `docs/dev/ENGINEERING_LOG.md`
