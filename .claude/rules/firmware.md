...
- `firmware/main/` is the only real application code today: `main.cpp` (startup + UART engine init), `ss_compass`, `ss_diag`, `ss_uart_engine`, `ss_tasks`, `ss_panic_guard`, `ss_recovery`, `ss_memwatch`, `ss_nvs`, `ss_bootmark`, `ss_version`, `Kconfig.projbuild`.
- `firmware/components/`: real components are `ss_hal` (header-only contracts; exports the selected board's `board_config.h`), `ss_log` (S-02-007), `ss_power` (S-03-001), `ss_input` (S-03-004), and `ss_display` (S-03-006, retires ss_display_boot). Remaining `ss_*` components are empty epic-gated scaffolds — first code in one means creating its `CMakeLists.txt`.
