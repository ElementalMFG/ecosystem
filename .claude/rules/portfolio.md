...
- Park-reason rule (generator-enforced, 2026-07-09): `IN_REVIEW` requires the park reason in `- Deps:` (which clause is unproven + what supplies the evidence) plus a row in `docs/dev/EVIDENCE_RUNSHEET.md` in the same commit; `BLOCKED` requires a `BLOCKED <date>: <reason>` annotation. `IN_REVIEW` = merged + tier-review-complete, AC evidence pending — NOT pending code review. Flip to `DONE` only on linked evidence (doc 00 §8).
