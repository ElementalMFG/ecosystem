...
// ---------------------------------------------------------------------------
struct ili_cmd_t { uint8_t cmd; uint8_t data[16]; uint8_t len; uint16_t delay_ms; };
...
esp_err_t ss_display_boot_init(void)
{
    ESP_LOGI(TAG, "ILI9488 bring-up: SPI%d SCLK=%d MOSI=%d DC=%d CS=%d BL=%d @%d Hz",
             int(SS_LCD_SPI_HOST) + 1, SS_LCD_PIN_SCLK, SS_LCD_PIN_MOSI,
             SS_LCD_PIN_DC, SS_LCD_PIN_CS, SS_LCD_PIN_BL, SS_LCD_SPI_FREQ_HZ);
    ...
    ESP_RETURN_ON_ERROR(spi_bus_initialize(SS_LCD_SPI_HOST, &bus, SPI_DMA_CH_AUTO),
                        TAG, "spi bus");
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_spi(SS_LCD_SPI_HOST, &io_cfg, &s_io),
                        TAG, "panel io");

    // 3. Conversion scratch (DMA-capable internal RAM).
    s_chunk = static_cast<uint8_t*>(
        heap_caps_malloc(kChunkPx * 3, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL));
    ...
    for (const auto& c : kInitSeq) {
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(s_io, c.cmd,
                            c.len ? c.data : nullptr, c.len), TAG, "init cmd");
                               ...
                               const uint16_t* px_rgb565)
{
    ...
    while (remaining) {
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_color(s_io, cmd, s_chunk, n * 3),
                            TAG, "tx color");
        src += n; remaining -= n; cmd = 0x3C;
    }
    return ESP_OK;
}
