...
| `.claude/skills/story-run/` | The standard story execution loop — select → elaborate (`Tasks:`/`Deps:` per methodology §2.7) → tier-route via §5/§7/§11 → implement → verify AC with evidence → status + index regen → `Story:` trailer commit — so tier lookup and AC verification happen every run, not when remembered | Same; §2.7 elaboration is additionally hard-enforced by `tools/gen-stories-index.py --check` in CI |
| `.claude/settings.json` `permissions` | Gate commands pre-approved; hand-edits to the generated `STORIES_INDEX.md` denied — the "regenerate, never hand-edit" rule is mechanical, not remembered | `permissions.allow` / `permissions.deny` |
