...
### S-14-020 — Inbound voicemail-to-text bridge
As an off-grid subscriber I want inbound calls to my number answered and delivered to me as transcribed text so that callers can reach me even over a LoRa-only link, without SS-SP hosting interconnected voice.
- AC: a call to the bridged number is answered by the gateway, recorded and transcribed on-device/edge (sherpa-onnx / whisper.cpp per the dossier §5), and delivered as an LXMF message to the mapped identity; because it is async text delivery it works for LoRa-only users (no live-voice path required); the feature stays off the interconnected-VoIP voice line (inbound-only, no user-originated PSTN voice) so it attracts only A2P-style obligations, not E911/CALEA; transcription runs offline on the gateway (no cloud dependency); the plaintext boundary (audio + transcript are cleartext at the gateway) is documented and passes wg-security review
- Meta: Shard=— | Type=Feature | Size=L | Prio=P2 | Status=DRAFT | SKU=A,O | PRD=F-TEL-05 | Const=C-08
- Tasks: spec inbound-answer → record → transcribe → LXMF-deliver flow · design gateway STT integration (sherpa-onnx/whisper.cpp) + TPI answer path · impl voicemail-to-text pipeline · test inbound call answered/transcribed/delivered over LoRa store-and-forward, offline STT, no-live-voice regulatory boundary · docs voicemail-to-text capability + plaintext-boundary note for wg-security
- Deps: D-0025, `12_TELEPHONY_AND_UNIVERSAL_INTEROP_DOSSIER.md`, S-21-030

### S-14-021 — Live voice ↔ PSTN via SIP↔LXST bridge (IP bearers, BYO/self-host only)
As a bridge operator I want full-duplex calls between an SS-SP identity and a phone number so that live PSTN voice is available on IP bearers, carried on my own carrier account so the interconnected-VoIP regime stays off the project.
- AC: a SIP↔LXST bridge connects an SS-SP identity to a PSTN number with Opus↔G.711 transcode at the gateway (WebRTC/WHIP per the dossier §4.2), reusing the EPIC-14 Opus stack and interoperating with Reticulum's LXST/`rnphone` layer (interop tracked via S-19-027); it is IP-bearers-only (HaLow-gatewayed / Wi-Fi / future cellular) and never available to LoRa-only field units; it is offered only on Rungs 1–4 (operator's own carrier account) — never in the hosted Rung-0 offering — so E911/CALEA/USF/CPNI attach to the operator's CPaaS/carrier, not SS-SP (F-TEL-08); STIR/SHAKEN verification is applied as a weighted signal, never a gate; a prominent "not for emergency calling" notice is shown and no 911/112 termination is implied (SOS remains bearer-flood per C-05)
- Meta: Shard=— | Type=Feature | Size=XL | Prio=P3 | Status=DRAFT | SKU=A,O | PRD=F-TEL-06 | Const=C-05, C-08
- Tasks: spec SIP↔LXST bridge + IP-bearer-only/Rungs-1–4-only constraints · design Opus↔G.711 transcode at gateway (WebRTC/WHIP) + LXST/rnphone interop · impl bridge over S-21-030 TPI SIP adapter · test full-duplex PSTN call on IP bearer, LoRa-exclusion, operator-carrier E911 boundary, STIR/SHAKEN-as-signal · docs live-voice regulatory posture + not-for-emergency notice
- Deps: D-0025, `12_TELEPHONY_AND_UNIVERSAL_INTEROP_DOSSIER.md`, S-21-030, S-19-027, S-24-038
