...
### S-17-022 — Self-hosted telephony bridge on Home Gateway (Rung 2)
As a home-gateway owner I want to run the open reference telephony bridge on my own gateway against my own CPaaS/SIP account so that I get phone-network reachability with maximal sovereignty and SS-SP is "just software" in the path.
- AC: the Apache-2.0 reference bridge runs on the Home Gateway (EPIC-17) as a Rung-2 deployment, using the S-21-030 TPI against the operator's own CPaaS/SIP account (the same bridge code path as the hosted Rungs 0/1, only the deployment location differs); the operator owns the number and carrier account, so all carrier/10DLC obligations attach to them and SS-SP holds no telephony relationship; SMS/MMS↔LXMF text works over every bearer including LoRa; the bridge is ADD-only and never-degrades the mesh (F-CL-07) and is fully removable with zero effect on mesh messaging; setup requires running a plugin/service, not a PBX (Rung 3 is the full-stack tier); the plaintext boundary at the PSTN edge is shown honestly in the gateway admin surface (labelled not end-to-end past the bridge)
- Meta: Shard=— | Type=Feature | Size=L | Prio=P2 | Status=DRAFT | SKU=A,O | PRD=F-TEL-04 | Const=C-04, C-08
- Tasks: spec Rung-2 self-host bridge deployment on the gateway · design operator-credential config + TPI wiring + admin-surface plaintext-boundary display · impl bridge service package for the gateway · test SMS/MMS↔LXMF over own CPaaS/SIP, removability/never-degrade, LoRa path · docs Rung-2 self-host guide + sovereignty/liability posture
