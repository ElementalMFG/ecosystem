...
- Meta: Shard=— | Type=Feature | Size=L | Prio=P0 | Status=DRAFT | SKU=★ | PRD=F-TEL-01 | Const=C-05, C-08
- Tasks: spec TPI operations, addressing model, `tel:` scheme, capability flags · design carrier-neutral interface + adapter plug points · impl `tpi-cpaas` reference adapter · test contract conformance suite + adapter integration against a CPaaS sandbox · docs interface reference (pre/post-conditions, errors) + adapter-authoring guide
- Deps: D-0025, `12_TELEPHONY_AND_UNIVERSAL_INTEROP_DOSSIER.md`
