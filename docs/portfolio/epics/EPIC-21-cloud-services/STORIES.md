...
- Deps: D-0025, `12_TELEPHONY_AND_UNIVERSAL_INTEROP_DOSSIER.md`; BLOCKED 2026-07-13: frozen contract delivered (`cloud/telephony-bridge/TPI_CONTRACT.md`), but the `tpi-cpaas` reference-adapter impl+CI and CPaaS-sandbox conformance AC are blocked — `cloud/` services and cloud CI are unscaffolded (empty dirs) and no CPaaS sandbox exists; needs a cloud-services bootstrap story + a cloud CI lane + a CPaaS sandbox account before implementation.
