...
- Meta: Shard=A | Type=Feature | Size=S | Prio=P0 | Status=DONE | SKU=★ | PRD=— | Const=C-00
- Tasks: spec (declaration-extraction rules + ownership-map contract: every declared HAL function is implemented in-tree or owned by a real story) · design (regex extractor over `ss_hal_*.h`, in-tree definition scan across `firmware/**/*.c`, checked-in ownership map keyed by `function → owning story`, story-filing failure message) · impl (`tools/contract-audit.py` + `tools/contract-ownership.txt` seed map + `Makefile` audit wiring + `weekly-audit.yml`) · test (self-check: green on current tree; a synthetic unowned declaration fails with the filing instruction) · docs (CHANGELOG + ENGINEERING_LOG)
- Deps: — (operates on checked-in `ss_hal_*.h` contracts already in-tree; no blocking story)
