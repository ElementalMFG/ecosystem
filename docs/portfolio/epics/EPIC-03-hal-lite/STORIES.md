...
- Deps: S-03-033 (mic shares GPIO 9/3/10 with LoRa behind the GPIO45 mux — capture must acquire SS_MUX_MODE_MIC) DONE; on-hardware capture ACs park at IN_REVIEW until an attached Lite board (D-0013)
...
- Deps: S-03-011 (LoRa driver — on-target integration consumer that feeds RSSI/CAD samples and gates ss_lora_tx; NOT DONE, so on-target ACs land with it); external: ETSI EN 300 220 duty-cycle sub-band table — on-air ACs park at IN_REVIEW until S-03-011 + the D-0013 units
...
- Deps: S-02-008 (storage partition frozen, DONE); S-02-015 (on-target Unity harness — for the deferred power-cut torture test; IN_PROGRESS) — the torture-test AC parks at IN_REVIEW until S-02-015 + an attached board
...
- Deps: S-03-023/`HIL_TEST_PLAN_LITE.md` (reuses the rack topology + Wi-Fi AP/BLE central peer fixtures — DONE); S-03-014/S-03-015 (Wi-Fi STA connect + reconnect the soak drives — IN_REVIEW); S-03-016/S-03-017 (BLE GATT link-under-load observable — DRAFT, referenced not required to author the plan); external: D-0013 Lite hardware fleet (soak execution + measured packet-loss/throughput park at IN_REVIEW until that session, so this story lands IN_REVIEW not DONE)
...
- Meta: Shard=— | Type=Feature | Size=M | Prio=P0 | Status=IN_PROGRESS | SKU=L | PRD=— | Const=C-00,C-01
- Tasks: spec — magnetometer read + heading math contract: HMC5883L 3-axis mag (X→Z→Y register byte order, 0x1E, 15 Hz ODR from `board_config.h` `SS_MAG_*`), tilt-compensation from IMU accel (pitch/roll → fused heading via atan2), and the three-source policy (`SS_COMPASS_SRC_MAG_TILTCOMP` when mag+IMU, `SS_COMPASS_SRC_MAG_ONLY` flat-hold when mag alone, `SS_COMPASS_SRC_NONE` phone-fed when neither); part-agnostic across the documented HMC5883L per the D-0013 clause · design — pure IDF-free `ss_compass_core.[ch]` (raw mag+accel counts → heading degrees + source selection, no `esp_err`/I²C) + thin IDF glue `ss_compass.c` owning I²C0 probe/read and feeding the `ss_hal_imu.h` mag path (`mx/my/mz` + `ss_imu_heading_deg`), mirroring the `ss_gnss`/`ss_power` core/glue split · impl — new `firmware/components/ss_compass/{include/ss_compass_core.h,src/ss_compass_core.c,src/ss_compass.c,CMakeLists.txt}`, capability-gated via `ss_hal_has_cap(SS_CAP_MAGNETOMETER)`/`SS_CAP_IMU`; migrate `main/ss_compass.cpp` heading/tilt/source helpers to the shared core so the three documented behaviours are preserved with no regression; `ss_hal/**` untouched (mag path already in frozen `ss_hal_imu.h`) · test — host gtest `test_ss_compass_core` (tilt-comp vs flat-hold heading vectors, X→Z→Y byte order, source selection matrix for all three modes, wrap/quadrant coverage), lite CI build; on-hardware heading sanity-check pending an attached Lite dev-fleet unit · docs — impl-site C-01 §compass-note citation + changelog + ENGINEERING_LOG; exact Elecrow part (HMC5883L @0x1E vs QMC5883L @0x0D) recorded per the D-0013 clause at attachment, updating doc 01 + pin map first on any deviation
- Deps: S-03-027 (IN_REVIEW — `ss_gnss` core/glue component pattern reused), S-03-001 (DONE — `ss_power` core/glue precedent), `ss_hal_imu.h` mag path (already frozen in `ss_hal`), `ss_compass` scaffolding (`main/ss_compass.{h,cpp}` — heading/tilt/source migrated behind the shared core); on-hardware ACs (exact part/address confirmed at attachment per the D-0013 clause; heading sanity-checked on a dev-fleet unit) need an attached Lite board / the D-0013 session — story parks at IN_REVIEW until then
...
### S-03-047 — Variant capability profiles + per-variant CI build matrix (D-0023)
As a release engineer I want named variant profiles over the module flags so that every Elecrow-based SKU configuration builds and ships plug-and-play.
- AC: named profiles map to `CONFIG_SS_LITE_MOD_*` sets — at minimum `lite-lora` (SX1262 on the wireless header), `lite-halow-lora` (HaLow on the wireless header + LoRa on the UART0-pins port per D-0023), and provisional `alpha-v1` (HaLow + GNSS/compass + ESP32-C6 coprocessor) · every profile builds in CI from the same tree with no per-variant source forks · per-profile capability masks are asserted by the board-parity gate · a profile's absent modules cost zero flash/RAM (compiled out via the module flags) · the profile of record for each shipping variant is documented
- Meta: Shard=— | Type=Feature | Size=M | Prio=P1 | Status=DRAFT | SKU=L,A | PRD=— | Const=C-00

### S-03-048 — Boot-time module presence detection → runtime capability flags
As a firmware engineer I want fitted-module probing at boot so that one firmware image adapts to what is physically attached (plug-and-play principle, D-0023).
- AC: safe, side-effect-free presence probes per slot (SX1262 SPI version register; C6 UART handshake/version exchange; GNSS UART NMEA sniff; compass/IMU I²C who-am-I) · each probe result reconciles against the compile-time profile — a mismatch is logged and the capability flag cleared, never a boot failure · the detected set is persisted to NVS and exposed via the capability API · total probe budget stays within a stated ms bound inside the boot-time budget (S-02-010) · the undetectable-module policy (assume-fitted vs assume-absent) is stated per module
- Meta: Shard=— | Type=Feature | Size=M | Prio=P1 | Status=DRAFT | SKU=L,A | PRD=— | Const=C-00
