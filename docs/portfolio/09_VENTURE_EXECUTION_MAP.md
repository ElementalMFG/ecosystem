...
| VA-04 | Generate the security-intake keypair(s) — age **and** PGP — on human-custodied hardware (ideally a hardware token; never inside a dev container or CI). Publish fingerprints in `SECURITY.md` §PGP key and at `security.ss-sp.org`. Store revocation certificates offline. | S-01-005 final gate; vulnerability intake | $ | DONE — ed25519 PGP `10D6…9FDD` + age recipient generated 2026-07-06 on the maintainer's WSL host (not container/CI); fingerprints live in `SECURITY.md`; private keys, passphrase, and revocation cert in maintainer custody (`~/ss-sp-keys/`, offline copy pending); `security.ss-sp.org` publication pending D-0012 |
